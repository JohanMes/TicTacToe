////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
	Oren-Nayar, berekend per pixel
*/

PS_OUTPUT PerPixelOrenNayarPS(VS_OUTPUT_PERPIXELSPECULAR In) {
	PS_OUTPUT outPS;
	...
	for(int i = 0;i < numdirlights;i++) {
		...
		diffuse += GetDiffuseValueOrenNayar(color,In.normalworld,lightdir,In.toeye) * GetDirlightShadow(i,In.projcoord);
	}
	...
	for(i = 0;i < numpointlights;i++) {
		...
		diffuse += GetDiffuseValueOrenNayar(color/distance(lightpos,In.positionworld),In.normalworld,lightdir,In.toeye) * GetPointlightShadow(i,In.projcoord);
	}
	...
	for(i = 0;i < numspotlights;i++) {
		...
		diffuse += spotvalue * GetDiffuseValueOrenNayar(color/distance(lightpos,In.positionworld),In.normalworld,lightdir,In.toeye) * GetSpotlightShadow(i,In.projcoord);
	}
...
technique PerPixelOrenNayar {
	pass P0 {
		VertexShader = compile vs_3_0 PerPixelVS();
		PixelShader  = compile ps_3_0 PerPixelOrenNayarPS();
	}
}
