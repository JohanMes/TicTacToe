PS_OUTPUT PerPixelNormalSpecularAmbientPS(VS_OUTPUT_PERPIXELNORMALSPECULAR In) {
	PS_OUTPUT outPS;
	...
	outPS.color.rgb = lerp(texColor.rgb*(specular + diffuse + lightambient*tex2D(AmbientSampler,In.coord).r),fogcolor,fog);
...
technique PerPixelNormalSpecularAmbient {
	pass P0 {
		VertexShader = compile vs_3_0 PerPixelNormalSpecularVS();
		PixelShader  = compile ps_3_0 PerPixelNormalSpecularAmbientPS();
	}
}
