////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
	Minnaert, berekend per pixel
*/

PS_OUTPUT PerPixelMinnaertPS(VS_OUTPUT_PERPIXELSPECULAR In) {
	PS_OUTPUT outPS;
	...
	for(int i = 0;i < numdirlights;i++) {
		...
		diffuse += GetDiffuseValueMinnaert(color,In.normalworld,lightdir,In.toeye) * GetDirlightShadow(i,In.projcoord);
	}	
	...
	for(i = 0;i < numpointlights;i++) {
		...
		diffuse += GetDiffuseValueMinnaert(color/distance(lightpos,In.positionworld),In.normalworld,lightdir,In.toeye) * GetPointlightShadow(i,In.projcoord);
	}
	...
	for(i = 0;i < numspotlights;i++) {
		...
		diffuse += spotvalue * GetDiffuseValueMinnaert(color/distance(lightpos,In.positionworld),In.normalworld,lightdir,In.toeye) * GetSpotlightShadow(i,In.projcoord);
	}
...
technique PerPixelMinnaert {
	pass P0 {
		VertexShader = compile vs_3_0 PerPixelSpecularVS();
		PixelShader  = compile ps_3_0 PerPixelMinnaertPS();
	}
}
