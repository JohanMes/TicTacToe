...
PS_OUTPUT PerPixelMultitexMixerPS(VS_OUTPUT_PERPIXEL In) {
	PS_OUTPUT outPS;
	...
	// We halen gewichten uit een texture
	float3 mixer = tex2D(ParallaxSampler,In.coord).xyz;

	float4 tex1 = mixer.x*tex2D(DiffuseSampler, In.coord*materialtiling);
	float4 tex2 = mixer.y*tex2D(SpecularSampler,In.coord*materialtiling);
	float4 tex3 = mixer.z*tex2D(NormalSampler,  In.coord*materialtiling);
...
technique PerPixelMultitexMixer {
	pass P0 {
		VertexShader = compile vs_3_0 PerPixelVS();
		PixelShader  = compile ps_3_0 PerPixelMultitexMixerPS();
	}
}
